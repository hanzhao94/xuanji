...
支持：
- SenseChat-5 / SenseChat-5-Cantonese
- SenseChat-32K / SenseChat-4 / SenseChat-128K
- 流式SSE
...
class SensenovaAdapter(BaseLLMAdapter):
    """商汤适配器（SenseNova）"""
    
    CHAT_MODELS = {"SenseChat-5", "SenseChat-5-Cantonese", "SenseChat-32K", "SenseChat-4", "SenseChat-128K"}
    ...
    def __init__(self, name: str, config: Dict[str, Any], **kwargs):
        ...
        self.base_url = config.get("base_url", "https://api.sensenova.cn/compatible-mode/v1").rstrip("/")
        self.api_key = config.get("api_key", "")
        self.model = config.get("model") or config.get("default_model", "SenseChat-5")
    ...
    async def embed(self, text: str, model: Optional[str] = None) -> List[float]:
        raise NotImplementedError(f"[{self.name}] Sensenova embedding via separate endpoint")
    
    def __repr__(self) -> str:
        status = "✓" if self.available else "✗"
        return f"<Sensenova [{status}] {self.model}>"
