...
class YiAdapter(BaseLLMAdapter):
    """零一万物适配器"""
    
    CHAT_MODELS = {"yi-lightning", "yi-large", "yi-medium", "yi-spark", "yi-large-fc"}
    ...
    def __init__(self, name: str, config: Dict[str, Any], **kwargs):
        ...
        self.base_url = config.get("base_url", "https://api.lingyiwanwu.com/v1").rstrip("/")
        self.api_key = config.get("api_key", "")
        self.model = config.get("model") or config.get("default_model", "yi-lightning")
    ...
    async def embed(self, text: str, model: Optional[str] = None) -> List[float]:
        raise NotImplementedError(f"[{self.name}] Yi does not support embedding")
    
    def __repr__(self) -> str:
        status = "✓" if self.available else "✗"
        return f"<Yi [{status}] {self.model}>"
