...
class BaichuanAdapter(BaseLLMAdapter):
    ...
    CHAT_MODELS = {"Baichuan4", "Baichuan4-Air", "Baichuan4-Turbo", "Baichuan3"}
    EMBED_MODELS = {"embedding-v3"}
    ...
    def __init__(self, name: str, config: Dict[str, Any], **kwargs):
        ...
        self.base_url = config.get("base_url", "https://api.baichuan-ai.com/v1").rstrip("/")
        self.api_key = config.get("api_key", "")
        self.model = config.get("model") or config.get("default_model", "Baichuan4")
    ...
    async def embed(self, text: str, model: Optional[str] = None) -> List[float]:
        ...
        headers = self._build_headers()
        body = {"model": model or self.config.get("embed_model", "embedding-v3"), "input": text}
        data = json.dumps(body, ensure_ascii=False).encode("utf-8")
    ...
    def __repr__(self) -> str:
        ...
        return f"<Baichuan [{status}] {self.model}>"
