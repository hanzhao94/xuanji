...
class MoonshotAdapter(BaseLLMAdapter):
    ...
    OpenAI兼容API，支持8k/32k/128k上下文。
    """
    
    CHAT_MODELS = {"moonshot-v1-8k", "moonshot-v1-32k", "moonshot-v1-128k"}
    EMBED_MODELS = {"moonshot-v1-embedding"}
    ...
    def __init__(self, name: str, config: Dict[str, Any], **kwargs):
        ...
        self.base_url = config.get("base_url", "https://api.moonshot.cn/v1").rstrip("/")
        self.api_key = config.get("api_key", "")
        self.model = config.get("model") or config.get("default_model", "moonshot-v1-8k")
    ...
    async def embed(self, text: str, model: Optional[str] = None) -> List[float]:
        ...
        headers = self._build_headers()
        body = {"model": model or self.config.get("embed_model", "moonshot-v1-embedding"), "input": text}
        data = json.dumps(body, ensure_ascii=False).encode("utf-8")
    ...
    def __repr__(self) -> str:
        ...
        return f"<Moonshot [{status}] {self.model}>"
