...
class GroqAdapter(BaseLLMAdapter):
    ...
    def __init__(self, name: str, config: Dict[str, Any], **kwargs):
        ...
        self.base_url = config.get("base_url", "https://api.groq.com/openai/v1").rstrip("/")
        self.api_key = config.get("api_key", "")
        self.model = config.get("model") or config.get("default_model", "llama-3.3-70b-versatile")
    ...
    async def ping(self) -> bool:
        try:
            await asyncio.wait_for(
                ...
                timeout=min(self.timeout, 15),  # Groq速度快，ping超时短
            )
            self.available = True
    ...
    async def embed(self, text: str, model: Optional[str] = None) -> List[float]:
        raise NotImplementedError(f"[{self.name}] Groq does not support embedding")
    
    def __repr__(self) -> str:
        status = "✓" if self.available else "✗"
        return f"<Groq [{status}] ⚡{self.model}>"
