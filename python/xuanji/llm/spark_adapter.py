...
class SparkAdapter(BaseLLMAdapter):
    ...
    CHAT_MODELS = {"generalv3.5", "generalv3.0", "generalv2", "max-balance"}
    ...
    def __init__(self, name: str, config: Dict[str, Any], **kwargs):
        ...
        self.base_url = config.get("base_url", "https://spark-api-open.xf-yun.com/v1").rstrip("/")
        self.api_key = config.get("api_key", "")
        self.model = config.get("model") or config.get("default_model", "generalv3.5")
    ...
    async def embed(self, text: str, model: Optional[str] = None) -> List[float]:
        raise NotImplementedError(f"[{self.name}] Spark does not support embedding")
    
    def __repr__(self) -> str:
        status = "✓" if self.available else "✗"
        return f"<Spark [{status}] {self.model}>"
