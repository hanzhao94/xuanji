...
class MinimaxAdapter(BaseLLMAdapter):
    ...
    def __init__(self, name: str, config: Dict[str, Any], **kwargs):
        ...
        self.base_url = config.get("base_url", "https://api.minimax.chat/v1").rstrip("/")
        self.api_key = config.get("api_key", "")
        self.model = config.get("model") or config.get("default_model", "MiniMax-Text-01")
    ...
    def _handle_http_error(self, e: urllib.error.HTTPError):
        try:
            ...
            err_msg = err_json.get("error", {}).get("message", "") or err_json.get("detail", "") or err_body[:500]
        except Exception:
            err_msg = str(e)
    ...
    async def embed(self, text: str, model: Optional[str] = None) -> List[float]:
        raise NotImplementedError(f"[{self.name}] MiniMax does not support embedding")
    
    def __repr__(self) -> str:
        status = "✓" if self.available else "✗"
        return f"<Minimax [{status}] {self.model}>"
